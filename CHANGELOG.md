...
- **`200 acknowledged` on an ingest pipeline now means "we will run this", and
  ...
  **Known gaps, stated rather than closed:** `_simulate` interprets the stored
  ...
  returns the ES body with no runnability signal, so an operator only learns at
  write time; a write addressed to an **alias** does not pick up the backing
  index's `default_pipeline`; and `_update_by_query` reports a `drop`
