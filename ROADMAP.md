...
- Full-text search (BM25) and **50 query types**. The list is no longer maintained by hand here: it is generated from `xerj_query::parser::SUPPORTED_QUERY_TYPES`, printed in full in [engine/README.md](./engine/README.md#query-types-supported) and [llms-full.txt](https://xerj.org/llms-full.txt), and pinned to `parse_query`'s dispatch table by `parser::tests::dispatch_table_matches_capability_manifest`. Two further keys — `has_child` and `has_parent` — are recognised and **rejected with a 400**, and are listed as such in the same places (issue #211).
  **Honest caveat, unchanged:** those 50 are the *dispatch* surface — every one of them parses, plans and executes, which is not a claim that all 50 are semantically faithful to ES. The known divergences are enumerated under *Partial / in progress* below, and the ES-YAML conformance suite is the measured answer.
- **Aggregations: 62 types**, likewise generated from `xerj_engine::aggs::SUPPORTED_AGG_TYPES` and printed in [engine/README.md](./engine/README.md#aggregation-types-supported) and [llms-full.txt](https://xerj.org/llms-full.txt). This includes the full **pipeline family** (`avg_bucket`, `sum_bucket`, `min_bucket`, `max_bucket`, `stats_bucket`, `extended_stats_bucket`, `percentiles_bucket`, `derivative`, `cumulative_sum`, `serial_diff`, `moving_avg`, `moving_fn`, `bucket_script`, `bucket_selector`, `bucket_sort`), which was absent from every published list until issue #211 — a reviewer reading those lists concluded XERJ had no pipeline aggregations and nearly filed a roadmap item to build them a second time. All aggregations are exact (no HLL, no sampling). `weighted_avg` is **not** among the 62 — see *Aggregations that are stubbed or silently degrade*.
